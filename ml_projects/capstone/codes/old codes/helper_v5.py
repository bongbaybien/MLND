...
def build_model(name, kernel_initializer=TruncatedNormal(), kernel_regularizer=None, batch_norm=False):
	image_shape = (48,48,1)
	n_classes = 7
	model_name = name
...
def train(x_train, y_train_onehot, x_test, y_test_onehot, x_private, y_private_onehot
	...
	checkpointer = ModelCheckpoint(filepath=cp_name+'_{epoch:02d}-{val_loss:.2f}.hdf5', monitor=cp_monitor)
